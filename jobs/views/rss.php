<? foreach( $GLOBALS['jobs']->items as $job) { ?>
	<item>
		...
		<link>https://<?= $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] ?>?action=JobPosting&amp;id=<?= $job->id ?></link>
		<guid isPermaLink="false">https://<?= $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] ?>?action=JobPosting&amp;id=<?= $job->id ?></guid>
